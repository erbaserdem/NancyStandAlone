    ...
    public class Validation : CheckExisting
    {
        ...
        public virtual bool ValidateUserName(out string message)
        {
            ...
            if (_userToValidate.UserName.Length > 25)
            {
             ...
             if (CheckForExistingUserName().Count != 0)
             {
                message += "UserName " + _userToValidate.UserName + "  is already in use ";
                 return false;
        ...
        public virtual bool ValidateEmail(out string message)
        {
            ...
            if (CheckForExistingEmail().Count != 0)
            {
                message += "Email " + _userToValidate.Email + "  is already in use ";
                return false;
            } // check if that username exists
        ...
        public virtual bool ValidateId(out string message)
        {
            ...
            if (CheckForExistingId().Count != 0)
            {
                message += "Id " + _userToValidate.Id + "  is already in use ";
                return false;
            } // check if that username exists
